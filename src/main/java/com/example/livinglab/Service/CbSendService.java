...
public class CbSendService {
    ...
    public List<Map<String, Object>> getAllMarketDetails() {
        List<Market> markets = marketRepository.findAll();  // Market 엔티티 리스트 가져오기
        return markets.stream()
                .map(market -> {
                    Map<String, Object> marketDetails = new HashMap<>();
                    marketDetails.put("id", market.getMarketcode()); // Market의 ID
                    marketDetails.put("name", market.getMarketname()); // Market의 이름
